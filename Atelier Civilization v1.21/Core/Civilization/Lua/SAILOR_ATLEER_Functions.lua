...
function Sailor_Atleer_Random_Resource(playerID, PlotX, PlotY, resourceID, cityID)
	...
	local pPlayer = Players[playerID]
		...
		if iNumberofCities > 0 then
			...
			for i, pIterCity in pPlayerCities:Members() do
				if i == iRandom then
					print("//// ", pIterCity:GetName(), "rolls the resource die!")
									...
									for i, pResource in ipairs(tValidResources) do
										...
										print("//// d100 Roll = ", iRandom4)
											...
											ResourceBuilder.SetResourceType(pTile, iResource, 1)
												...
												--[[iLocalPlayer = Game.GetLocalPlayer()
												print(iLocalPlayer)
												print(playerID)
												if (playerID == iLocalPlayer) then
													LuaEvents.Custom_StatusMessage(Locale.Lookup(pResourceRolled, " synthesized in ", pIterCity:GetName()), 5, ReportingStatusTypes.DEFAULT)
												end]]--
											end
										end
									end
